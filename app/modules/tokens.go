...
// DefaultTokenIdentifier is the identifier of the token the server ensures
// at start.
const DefaultTokenIdentifier = "default"

// EnsureDefaultToken is the server-start rule of the template: when no
// default token exists, an existing token with the identifier "default" is
// made the default, otherwise a new token "default" (no expiry) is created
// and returned so the caller logs its value once. Deleting the default
// never promotes another token. Nothing is created anywhere else.
func EnsureDefaultToken(db *sql.DB) (created *models.Token, promoted bool, err error) {
	if _, err := models.GetDefaultToken(db); err == nil {
		return nil, false, nil
	} else if err != sql.ErrNoRows {
		return nil, false, err
	}
	if _, err := models.GetTokenByIdentifier(db, DefaultTokenIdentifier); err == nil {
		return nil, true, models.SetDefaultToken(db, DefaultTokenIdentifier)
	} else if err != sql.ErrNoRows {
		return nil, false, err
	}
	tok, err := CreateToken(db, DefaultTokenIdentifier, DefaultTokenIdentifier, sql.NullTime{}, true)
	if err != nil {
		return nil, false, err
	}
